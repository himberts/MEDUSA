...
function logrunningresource( $uuid, $resource, $nodes, $nodesppn, $error_json_exit = false ) {
    ...
    if ( 0 && 
         ...
         $update = [];
         $update[ "_id" ] = "mxray:$resource:" . $doc[ 'xsedeproject' ];
...
function jobcancel( $jobs, $error_json_exit = false, $is_admin = false ) {
    ...
    $appconfig = "/opt/genapp/mxray/appconfig.json";
    ...
    foreach ( $runs as $v ) {
       ...
       if ( isset( $v[ 'resource' ] ) ) {
           if ( $v[ 'resource' ] == "openstack" &&
                isset( $v[ 'nodes' ] ) ) {
               require_once "/var/www/html/mxray/openstack/os_delete.php";
    ...
    foreach ( $tokill as $k => $v ) {
        if ( !isset( $appjsona[ 'resources' ][ $k ] ) ) {
            ...
            $kill = $appjsona[ 'resources' ][ $k ] . " /var/www/html/mxray/util/ga_killprocs.pl /var/www/html/mxray/log $k all " . implode( ' ', $v );
    ...
    foreach ( $tokillparent as $k => $v ) {
        if ( !isset( $appjsona[ 'resources' ][ $k ] ) ) {
            ...
            $kill = $appjsona[ 'resources' ][ $k ] . " /var/www/html/mxray/util/ga_killprocs.pl /var/www/html/mxray/log $k child " . implode( ' ', $v );

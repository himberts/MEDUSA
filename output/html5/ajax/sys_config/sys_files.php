...
$modjson = json_decode( '{"executable":"sys_files","fields":[{"id":"label1","label":"Files","role":"input","type":"label"}],"label":"files","moduleid":"sys_files","noreset":"true","resource":"local"}' );
$inputs_req = $_REQUEST;
$validation_inputs = ga_sanitize_validate( $modjson, $inputs_req, 'sys_files' );
...
$dir = "/var/www/html/mxray/results/users/";
...
//       error_log( print_r( $results, true ) . "\n", 3, "/tmp/mylog" );
       ...
       if ( isset( $do_cmd ) )
       {
          ...
          $deldir = "/var/www/html/mxray/deleted/users/" . $GLOBALS[ 'logon' ] . "/" . $uniq;
          $tries = 0;
          while ( file_exists( $deldir ) )
          {
              $deldir = "/var/www/html/mxray/deleted/users/" . $GLOBALS[ 'logon' ] . "/" . bin2hex( openssl_random_pseudo_bytes ( 6, $cstrong ) );
              if ( ++$tries > 100 )
              {

...
$modjson = json_decode( '{"executable":"sys_pull","fields":[{"default":"header3","id":"label1","label":"Pull","posthline":"true","role":"input","type":"label"},{"id":"project","role":"input","type":"integer"},{"id":"xsedeproject","role":"input","type":"integer"},{"id":"datetime","role":"input","type":"integer"},{"id":"email","role":"input","type":"integer"},{"id":"visible","role":"input","type":"integer"}],"label":"Pull","moduleid":"sys_pull","noreset":"true","resource":"local","submit_label":"Pull"}' );
#$modjson = [];
$inputs_req = $_REQUEST;
$validation_inputs = ga_sanitize_validate( $modjson, $inputs_req, 'sys_pull' );
...
if ( $_REQUEST[ '_logon' ] != $_SESSION[ $window ][ 'logon' ] )
{
   ...
   mymail( $json->mail->admin, 'security alert mxray', "session timeout or possible security breach attempt on mxray\n" .
                   ...
                   if ( $keok && $kedoc ) {
                       ...
                       switch ( $k ) {
                           case "color:body:color" :
                               $results[ $k ] = rgbtohex( "43,29,14" );
                           break;
                           case "color:body:background" :
                               $results[ $k ] = rgbtohex( "195,200,200" );
                           break;
                           default :
                               $results[ $k ] = "";
                           break;

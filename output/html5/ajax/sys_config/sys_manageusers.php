...
function handle_request() {
    ...
    switch( $_REQUEST[ '_cmd' ] ) {
        case "remove" : {
            $mailuser = "Your account on http://" . $app->hostname . "/mxray has been Removed.";
        ...
        case "approve" : {
            $mailuser = "Your request for an account on http://" . $app->hostname . "/mxray has been Approved.";
            if ( isset( $doc[ 'needsemailverification' ] ) ) {
                $mailuser .= "\nPlease verify your email address by visiting this link:\n http://" . $app->hostname . "/mxray/ajax/sys_config/sys_register_backend.php?_r=" . $doc[ '_id' ];
            } else {
                $mailuser .= "\nYou can now logon.";
            }
            $update = [ 
        ...
        case "deny" : {
            $mailuser = "Your request for an account on http://" . $app->hostname . "/mxray has been Denied";
            $update = [
                '$unset' => [ 
                    "approvalid" => ""
                    ,"denyid" => "" 
        ...
        case "suspend" : {
            $mailuser = "Your account on http://" . $app->hostname . "/mxray has been Suspended";
            $update = [ 
                '$set' => [
        ...
        case "activate" : {
            $mailuser = "Your account on http://" . $app->hostname . "/mxray has been Activated";
            $update = [ 
                '$unset' => [ 
                    'suspended' => 1 
                ],
        ...
        case "resend_email_verification" : {
            ...
            $mailsubject = "[mxray][email verify request]";
        }
        break;
    ...
    if ( isset( $remove ) ) {
        ...
        if ( isset( $removedata ) ) {
            $dir = "/var/www/html/mxray/results/users/" . $doc[ 'name' ];
            if ( file_exists( $dir ) ) {
                $cstrong = true;
                $tmplen = 6;
                $uniq = bin2hex( openssl_random_pseudo_bytes ( $tmplen, $cstrong ) );
                $deldir = "/var/www/html/mxray/removed/$uniq/" . $doc[ 'name' ];
                $tries = 0;
    ...
    if ( isset( $mailuser ) ) {
        mymail( $doc[ 'email' ], isset( $mailsubject ) ? $mailsubject : "[mxray][account status update]", $mailuser );
    }

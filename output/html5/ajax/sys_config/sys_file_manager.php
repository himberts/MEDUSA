...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/mxray/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/mxray/bin/sys_file_manager";
...
$GLOBALS[ 'module'    ] = "sys_file_manager";
$GLOBALS[ 'jobweight' ] = floatval( "__jobweight__" );
$GLOBALS[ 'menu'      ] = "sys_config";
...
$_REQUEST[ '_menu' ]        = "sys_config";
$_REQUEST[ '_module' ]      = "sys_file_manager";
     ...
     !isset( $useresource ) )
{
   ...
   if ( strlen( $cmdprefix ) > 1 ) {
      ...
      $cmdprefix = str_replace( "_" . "_menu:id__", "sys_config", $cmdprefix );
      $cmdprefix = str_replace( "_" . "_menu:modules:id__", "sys_file_manager", $cmdprefix );
...
if ( !sizeof( $_FILES ) ) {
   ...
   foreach ( $_REQUEST as $k=>$v ) {
      if ( !strncmp( $k, $selalt, $lenselalt ) ) {
          ...
          error_log( "sys_file_manager no files but found _selaltval_ with key $tmp_key\n", 3, "/tmp/mylog_selalt" );
      }
   }
   if ( $found_selalt ) {
       error_log( "sys_file_manager request\n" . print_r( $_REQUEST, true ) . "\n", 3, "/tmp/mylog_selalt" );
       error_log( "files\n" . print_r( $_FILES, true ) . "\n", 3, "/tmp/mylog_selalt" );
   }
}
...
if ( sizeof( $_FILES ) ) {
    ...
    $module_json = json_decode( '{"docrootexecutable":"ajax/sys_config/sys_file_manager_run.php","executable":"sys_file_manager","fields":[{"default":"header3","id":"label1","label":"Files","posthline":"true","role":"input","type":"label"},{"help":"This is the date and time from the file system on the server at the time you opened this window","id":"serverdate","label":"Server date","pull":"datetime","role":"input","type":"label"},{"id":"selectedfiles","label":"User file tree","role":"input","type":"ftree"},{"default":"tar","help":"You can select the compression type here.  Select <b>none</b> for a list of individual file links","id":"compression","label":"Compression type","name":"compression","role":"input","type":"listbox","values":"none~no~uncompressed tarball~tar~gzip tarball~gz~bzip2 tarball~bz2~xz tarball~xz~zipped~zip"},{"id":"status","label":"","multiple":"true","role":"output","type":"html"},{"id":"outfiles","label":"","multiple":"true","role":"output","type":"file"}],"height":"65vh","label":"Files Manager","modal":"true","moduleid":"sys_file_manager","nofcrefresh":"true","nojobcontrol":"true","noreset":"true","resetonload":"true","resource":"local","submit_label":"Download","uniquedir":"true"}' );
    $required_files = [];
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "mxray sys_config sys_file_manager last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "mxray sys_config sys_file_manager target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "mxray sys_config sys_file_manager target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if ( $cmdprefix == "airavatarun" ||
          ...
          if ( substr( $cmdprefix, 0, 5 ) != "abaco" ) {
              $cmd .= $cmdprefix == "oscluster" ? " sys_file_manager" : " sys_file_manager";
          }
          $cmd .= " '$json'"; 
      } else {

...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in admin sys_manage_users.\n" );
...
$modjson = json_decode( '{"autosubmit":"true","docrootexecutable":"ajax/sys_config/sys_manageusers.php","executable":"sys_manage_users","fields":[{"id":"sysuserreport","label":"","role":"output","type":"html"}],"label":"Sys_Manage_Users","moduleid":"sys_manage_users","nojobcontrol":"true","noreset":"true","resource":"local","submit_label":"Refresh","submitpolicy":"all","uniquedir":"on"}' );
$inputs_req = $_REQUEST;

### turn off validation until undefined variable bug fix
# $validation_inputs = ga_sanitize_validate( $modjson, $inputs_req, 'sys_manage_users' );
...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/mxray/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/mxray/bin/sys_manage_users";
...
$GLOBALS[ 'module'    ] = "sys_manage_users";
...
$_REQUEST[ '_module' ]      = "sys_manage_users";
     ...
     !isset( $useresource ) )
{
   ...
   if ( strlen( $cmdprefix ) > 1 ) {
      ...
      $cmdprefix = str_replace( "_" . "_menu:modules:id__", "sys_manage_users", $cmdprefix );
...
if ( !sizeof( $_FILES ) ) {
   ...
   foreach ( $_REQUEST as $k=>$v ) {
      if ( !strncmp( $k, $selalt, $lenselalt ) ) {
          ...
          error_log( "sys_manage_users no files but found _selaltval_ with key $tmp_key\n", 3, "/tmp/mylog_selalt" );
      }
   }
   if ( $found_selalt ) {
       error_log( "sys_manage_users request\n" . print_r( $_REQUEST, true ) . "\n", 3, "/tmp/mylog_selalt" );
       error_log( "files\n" . print_r( $_FILES, true ) . "\n", 3, "/tmp/mylog_selalt" );
   }
}
...
if ( sizeof( $_FILES ) ) {
    ...
    $module_json = json_decode( '{"autosubmit":"true","docrootexecutable":"ajax/sys_config/sys_manageusers.php","executable":"sys_manage_users","fields":[{"id":"sysuserreport","label":"","role":"output","type":"html"}],"label":"Sys_Manage_Users","moduleid":"sys_manage_users","nojobcontrol":"true","noreset":"true","resource":"local","submit_label":"Refresh","submitpolicy":"all","uniquedir":"on"}' );
    $required_files = [];
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "mxray admin sys_manage_users last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "mxray admin sys_manage_users target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "mxray admin sys_manage_users target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if ( $cmdprefix == "airavatarun" ||
          ...
          if ( substr( $cmdprefix, 0, 5 ) != "abaco" ) {
              $cmd .= $cmdprefix == "oscluster" ? " sys_manage_users" : " sys_manage_users";
          }
          $cmd .= " '$json'"; 
      } else {

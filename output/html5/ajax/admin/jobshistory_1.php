...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in admin jobshistory_1.\n" );
...
# $validation_inputs = ga_sanitize_validate( $modjson, $inputs_req, 'jobshistory_1' );
...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/mxray/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/mxray/bin/jobshistory_1";
...
$_REQUEST[ '_module' ]      = "jobshistory_1";
     ...
     !isset( $useresource ) )
{
   ...
   if ( strlen( $cmdprefix ) > 1 ) {
      ...
      $cmdprefix = str_replace( "_" . "_menu:modules:id__", "jobshistory_1", $cmdprefix );
...
if ( !sizeof( $_FILES ) ) {
   ...
   foreach ( $_REQUEST as $k=>$v ) {
      if ( !strncmp( $k, $selalt, $lenselalt ) ) {
          ...
          error_log( "jobshistory_1 no files but found _selaltval_ with key $tmp_key\n", 3, "/tmp/mylog_selalt" );
      }
   }
   if ( $found_selalt ) {
       error_log( "jobshistory_1 request\n" . print_r( $_REQUEST, true ) . "\n", 3, "/tmp/mylog_selalt" );
       error_log( "files\n" . print_r( $_FILES, true ) . "\n", 3, "/tmp/mylog_selalt" );
   }
}
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "mxray admin jobshistory_1 last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "mxray admin jobshistory_1 target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "mxray admin jobshistory_1 target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if ( $cmdprefix == "airavatarun" ||
          ...
          if ( substr( $cmdprefix, 0, 5 ) != "abaco" ) {
              $cmd .= $cmdprefix == "oscluster" ? " jobshistory_1" : " jobshistory_1";
          }
          $cmd .= " '$json'"; 
      } else {

...
function phpmailer_mail( $to, $subject, $body, $attach = NULL, $attachdata = NULL ) {
    ...
    require '/var/www/html/mxray/vendor/phpmailer/phpmailer/src/Exception.php';
    require '/var/www/html/mxray/vendor/phpmailer/phpmailer/src/PHPMailer.php';
    require '/var/www/html/mxray/vendor/phpmailer/phpmailer/src/SMTP.php';
...
#        $mail->SMTPDebug = SMTP::DEBUG_SERVER;                     # Enable verbose debug output, comment this line in production
        ...
        $mail->setFrom( 'mxray@' . $json->mail->from );
        ...
        if ( isset( $attach ) && count( $attach ) ) {
            ...
            foreach ( $attach as $f ) {
                # $fn = str_replace( '/var/www/html/mxray/', '', $f );
                $fn = preg_replace( '/^.*\//', '', $f );
...
function mymail( $to, $subject, $body )
{
   ...
   $headers = array ('From' => 'mxray@' . $json->mail->from,
                     'To' => $to,
                     'Subject' => $subject );
...
{
   return admin_mail( "[mxray][internal error message]", $body );
}
